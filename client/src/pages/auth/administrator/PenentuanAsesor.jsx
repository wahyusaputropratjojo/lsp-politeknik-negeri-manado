...
export const PenentuanAsesor = () => {
  ...
  const [asesiTempatUjiKompetensi, setAsesiTempatUjiKompetensi] = useState(null);
  ...
  const { refetch } = useQuery({
    ...
    queryFn: async () => {
      return await axios.get(`/administrator/${id_user}/tempat-uji-kompetensi/asesi`);
    },
    onSuccess: async (data) => {
      const { tempat_uji_kompetensi: tempatUjiKompetensi } = await data.data.data;
                  ...
                  return (
                    <article key={id} className="flex flex-col gap-4">
                      {filteredAsesiSkemaSertifikasi.map((value, index) => {
                        const { id, asesi, tujuan_asesmen: tujuanAsesmen } = value;
                            ...
                            <AccordionItem value="item-1">
                              <AccordionTrigger>
                                <div className="flex w-full items-center gap-6">
                                  <div>
                                    <img
                                      src={asesi.user.url_profil_user}
                                  ...
                                  <div className="grid grid-flow-col grid-rows-2 items-center gap-x-12 gap-y-4">
                                    <div className="flex flex-col items-start">
                                      ...
                                      <p className="text-sm">{asesi.user.nama_lengkap}</p>
                                    </div>
                                    <div className="flex flex-col items-start">
                                      <div className="flex flex-col items-start">
                              ...
                              <AccordionContent>
                                <div className="flex flex-col gap-8 pt-6">
                                  <div className="flex flex-col gap-12">
                                    <PenentuanAsesorForm id={id} setIsRefetch={setIsRefetch} />
                                  </div>
                                </div>
                              </AccordionContent>
                            </AccordionItem>
...
const PenentuanAsesorForm = ({ id, setIsRefetch }) => {
  ...
  const [asesorTempatUjiKompetensi, setAsesorTempatUjiKompetensi] = useState(null);

  useQuery({
    queryKey: ["asesor-tempat-uji-kompetensi", id_user],
    queryFn: async () => {
      return await axios.get(`/administrator/${id_user}/tempat-uji-kompetensi/asesor`);
    },
    onSuccess: async (data) => {
      const { tempat_uji_kompetensi: tempatUjiKompetensi } = await data.data.data;
                ...
                <FormItem>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        disabled={isLoading}
                        similar={form.formState.errors?.id_asesor ? "input-error" : "input-primary"}
                        role="combobox"
                        ...
                        <p className="col-span-11 flex w-full items-start justify-self-start truncate">
                          {field.value
                            ? asesorTempatUjiKompetensi.find((data) => data.id === field.value)
                                ?.user.nama_lengkap
                            : ""}
                        </p>
