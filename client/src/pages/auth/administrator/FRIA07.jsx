...
export const FRIA07Administrator = () => {
  ...
  const [pertanyaanLisanData, setPertanyaanLisanData] = useState();
  const [idPertanyaanLisan, setIdPertanyaanLisan] = useState();

  const { refetch } = useQuery({
    queryKey: ["pertanyaan-lisan", idUnitKompetensi],
    queryFn: async () => {
      return await axios.get(
        `/skema-sertifikasi/unit-kompetensi/${idUnitKompetensi}/pertanyaan-lisan`,
      );
    },
    onSuccess: (data) => {
      setPertanyaanLisanData(data.data.data.pertanyaan_lisan);
    },
  });

  const { mutate: mutateDeletePertanyaanLisan } = useMutation({
    mutationFn: async (data) => {
      return await axios.delete(
        `/skema-sertifikasi/unit-kompetensi/pertanyaan-lisan/${idPertanyaanLisan}`,
        data,
      );
    },
    onSuccess: (data) => {
  ...
  const { mutate } = useMutation({
    mutationFn: async (data) => {
      return await axios.post("/skema-sertifikasi/unit-kompetensi/pertanyaan-lisan", data);
    },
    onSuccess: (data) => {
      toast({
        variant: "success",
    ...
    <section>
      <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          {!!pertanyaanLisanData && pertanyaanLisanData.length > 0 && (
            <div className="flex flex-col gap-4 rounded-lg bg-white p-12 shadow-lg">
              ...
              <div className="flex flex-col gap-2">
                <Table className="text-base">
                  ...
                  <TableBody>
                    {!!pertanyaanLisanData &&
                      pertanyaanLisanData.map((value, index) => {
                                    ...
                                    <AlertDialogFooter>
                                      <div className="flex w-full gap-4">
                                        ...
                                        <AlertDialogAction
                                          ...
                                          onClick={() => {
                                            setIdPertanyaanLisan(id);
                                            mutateDeletePertanyaanLisan();
                                          }}>
                                          Konfirmasi
                                        </AlertDialogAction>
                                      </div>
