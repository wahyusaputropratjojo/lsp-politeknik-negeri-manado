...
export const FormPertanyaanObservasi = ({ form }) => {
    ...
    <div key={item.id}>
      <Accordion type="single" collapsible>
        <AccordionItem value="item-1">
          ...
          <AccordionContent>
            <PertanyaanObservasi indexUnitKompetensi={index} form={form} />
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </div>
...
const PertanyaanObservasi = ({ indexUnitKompetensi, form }) => {
  const { fields, remove, prepend, append } = useFieldArray({
    name: `unit_kompetensi.${indexUnitKompetensi}.pertanyaan_observasi`,
    control: form.control,
  });

  const pertanyaanObservasiFields = fields.map((field, index) => {
    return (
      <div
        key={field.id}
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <FormField
            control={form.control}
            name={`unit_kompetensi.${indexUnitKompetensi}.pertanyaan_observasi.${index}.pertanyaan`}
            render={({ field }) => (
